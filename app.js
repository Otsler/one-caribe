...
function showSection(id) {
  ...
  // mostrar selector de sucursal en inventario solo para Admin o Principal
...
async function limpiar(tipo) {
  ...
  try {
    ...
    if (tipo === "inventario") {
      ...
      let all = await db.collection(tipo).get();
      let batch = [];
      all.forEach(doc => {
        let d = doc.data();
        if ((d.sucursal || "Principal") === suc) batch.push(doc.ref);
      });
      batch.forEach(ref => ref.delete());
      hideSpinner();
      return toast("✅ Eliminado");
    }
...
function crearUsuario() {
  ...
  db.collection("usuarios").add({ usuario:u, clave:c, rol:rol, sucursales });
  ...
  db.collection("usuarios").onSnapshot(snap => {
    ...
    snap.forEach(doc => {
      ...
      tablaUsuarios.innerHTML += `<tr>
        ...
        <td>${u.rol}</td>
        <td>${Array.isArray(u.sucursales) ? u.sucursales.join(", ") : (u.sucursal||"Principal")}</td>
        <td>
...
async function cargarSucursales() {
  ...
  db.collection("sucursales").onSnapshot(snap => {
    ...
    // actualizar el selector de vista de inventario
