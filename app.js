...
let _configLotes = { exigirLoteEntradas: false, validarLoteSalidas: false };
...
function _actualizarStockTraslado() {
  ...
  db.collection(col).doc(clave).get().then(doc => {
    ...
    el.style.color = stock > 0 ? "#16a34a" : "#dc2626";
  }).catch(() => { el.textContent = "Error consultando stock"; el.style.color = "#dc2626"; });
}
...
async function _renderResumenSucursales(cont) {
  ...
  try {
    let rows = await Promise.all(sucursales.map(async suc => {
      let [invSnap, trasSnap, estibasSnap] = await Promise.all([
        suc === "Principal"
        ...
        suc === "Principal"
          ? db.collection("estibas").get()
          : db.collection("estibas").where("sucursal","==",suc).get()
      ]);
      let estibasCfg = {};
      estibasSnap.forEach(doc => {
        let d = doc.data();
        if (suc === "Principal" && d.sucursal && d.sucursal !== "Principal") return;
        estibasCfg[d.producto + "_" + d.referencia] = d.pacas;
      });
      let pacas = 0;
      let estOcupadas = 0;
      invSnap.forEach(doc => {
        let x = doc.data();
        pacas += x.pacas || 0;
        let cfg = estibasCfg[x.producto + "_" + x.referencia];
        if (cfg) estOcupadas += (x.pacas || 0) / cfg;
      });
      let cap = _capacidades[suc] || 0;
      let pct = cap > 0 ? Math.min(100, estOcupadas / cap * 100).toFixed(1) + "%" : "—";
      let colorPct = "";
      if (cap > 0) {
        let p = estOcupadas / cap * 100;
...
async function salida() {
  ...
  if (_configLotes.validarLoteSalidas && !lote) return toast("❌ Debe ingresar el número de lote", "error");
  ...
  try {
    let doc;
    if (_configLotes.validarLoteSalidas && lote) {
      let [snapE, snapS, snapT, docInv] = await Promise.all([
        db.collection("entradas").where("producto","==",p).where("referencia","==",r).where("lote","==",lote).get(),
        db.collection("salidas").where("producto","==",p).where("referencia","==",r).where("lote","==",lote).get(),
        db.collection("traslados").where("producto","==",p).where("referencia","==",r).where("lote","==",lote).where("sucursalOrigen","==",suc).get(),
        db.collection(col).doc(clave).get()
      ]);
      let stockLote = 0;
      snapE.forEach(d => { let x = d.data(); if (!x.anulado && (x.sucursal||"Principal") === suc) stockLote += (x.pacas||0); });
      snapS.forEach(d => { let x = d.data(); if (!x.anulado && (x.sucursal||"Principal") === suc) stockLote -= (x.pacas||0); });
      snapT.forEach(d => { let x = d.data(); if (x.estado !== "rechazado") stockLote -= (x.pacas||0); });
      if (stockLote <= 0) { hideSpinner(); return toast(`❌ El lote "${lote}" no existe o no tiene stock disponible`, "error"); }
      if (stockLote < pac) { hideSpinner(); return toast(`❌ Stock insuficiente en lote "${lote}" — disponible: ${stockLote} pacas`, "error"); }
      doc = docInv;
    } else {
      doc = await db.collection(col).doc(clave).get();
    }
    ...
    await db.collection(col).doc(clave).update({ pacas: data.pacas - pac });
    pacasS.value = "";
...
async function _verInventarioConsolidado() {
  ...
  try {
    ...
    if (t) {
      t.innerHTML = "";
      _inventarioData.forEach(x => {
        let suc2 = x.sucursal || "Principal";
        t.innerHTML += `<tr style="cursor:pointer;" onclick="verLotesReferencia('${x.producto.replace(/'/g,"\\'")}','${x.referencia.replace(/'/g,"\\'")}','${suc2}')"><td>${suc2}</td><td>${x.producto}</td><td>${x.referencia}</td><td>${x.pacas}</td><td>—</td></tr>`;
      });
    }
...
function verInventario() {
  ...
  if (getViewSucursal() === "CONSOLIDADO") {
    let _debTimer = null;
    const _refresh = () => { clearTimeout(_debTimer); _debTimer = setTimeout(_verInventarioConsolidado, 350); };
    let _u1 = db.collection("inventario").onSnapshot(_refresh);
    let _u2 = db.collection("inventarioSucursales").onSnapshot(_refresh);
    _unsubInventario = () => { _u1(); _u2(); clearTimeout(_debTimer); };
    return;
  }
  ...
  _unsubInventario = query.onSnapshot(snap => {
    ...
    snap.forEach(doc => {
      ...
      totalPacas += x.pacas;
      tablaInventario.innerHTML += `<tr style="cursor:pointer;" onclick="verLotesReferencia('${x.producto.replace(/'/g,"\\'")}','${x.referencia.replace(/'/g,"\\'")}')">
        <td>${x.producto}</td><td>${x.referencia}</td><td>${x.pacas}</td>
...
async function verLotesReferencia(producto, referencia, suc) {
  suc = suc || getViewSucursal();
  showSpinner();
  let lotes = {};
  try {
    let [snapE, snapS, snapT] = await Promise.all([
      db.collection("entradas").where("producto","==",producto).where("referencia","==",referencia).get(),
      db.collection("salidas").where("producto","==",producto).where("referencia","==",referencia).get(),
      db.collection("traslados").where("producto","==",producto).where("referencia","==",referencia).where("sucursalOrigen","==",suc).get()
    ]);
    snapE.forEach(doc => {
      let x = doc.data();
      if (x.anulado || (x.sucursal || "Principal") !== suc) return;
      let l = x.lote || "Sin lote"; lotes[l] = (lotes[l] || 0) + (x.pacas || 0);
    });
    snapS.forEach(doc => {
      let x = doc.data();
      if (x.anulado || (x.sucursal || "Principal") !== suc) return;
    ...
    snapT.forEach(doc => {
      let x = doc.data();
      if (x.estado === "rechazado") return;
      let l = x.lote || "Sin lote"; lotes[l] = (lotes[l] || 0) - (x.pacas || 0);
    });
  } finally { hideSpinner(); }

  let filas = Object.entries(lotes)
    .filter(([, pac]) => pac > 0)
    .sort(([a], [b]) => a.localeCompare(b))
    .map(([lote, pac]) => `<tr><td>${lote}</td><td><b>${pac}</b></td></tr>`)
    .join("");

  document.getElementById("modalLotesTitulo").innerText = `${producto} · ${referencia} — ${suc}`;
  document.getElementById("modalLotesBody").innerHTML = filas || '<tr><td colspan="2" class="sin-datos">Sin lotes registrados</td></tr>';
...
function cargarConfigLotes() {
  db.collection("configuracion").doc("lotes").onSnapshot(doc => {
    if (doc.exists) _configLotes = { ...doc.data() };
    let chkE = document.getElementById("toggleLoteEntradas");
    let chkS = document.getElementById("toggleLoteSalidas");
    if (chkE) chkE.checked = !!_configLotes.exigirLoteEntradas;
    if (chkS) chkS.checked = !!_configLotes.validarLoteSalidas;
  });
}

function guardarConfigLotes() {
  let chkE = document.getElementById("toggleLoteEntradas");
  let chkS = document.getElementById("toggleLoteSalidas");
  _configLotes.exigirLoteEntradas = chkE ? chkE.checked : false;
  _configLotes.validarLoteSalidas = chkS ? chkS.checked : false;
  db.collection("configuracion").doc("lotes").set(_configLotes);
  toast("✅ Configuración de lotes guardada");
}
...
async function limpiar(tipo) {
  ...
  try {
    ...
    hideSpinner();
    toast(`✅ ${refs.length} registros eliminados`);
...
function crearPreEntrada() {
  ...
  if (!pac) return toast("Ingrese cantidad", "error");
  if (_configLotes.exigirLoteEntradas && !lote) return toast("❌ Debe ingresar el número de lote", "error");
...
async function crearTraslado() {
  ...
  try {
    let doc = await db.collection(col).doc(clave).get();
