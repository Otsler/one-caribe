  ...
  if (suc === "CONSOLIDADO") {
    Promise.all([db.collection("inventario").get(), db.collection("inventarioSucursales").get()])
      .then(([sP, sS]) => {
        ...
        <div><b style="font-size:22px">ONE CARIBE</b><div style="color:#555;font-size:13px">Inventario Consolidado — Todas las Sucursales</div></div>
        ...
        <div style="margin-top:30px;text-align:center;color:#555;font-size:12px;">© 2026 ONE CARIBE</div></body></html>`);
    ...
    Promise.all(promesas).then(filas => {
      ...
      <body>
        <div style="display:flex;justify-content:space-between;margin-bottom:20px;">
          <div><b style="font-size:22px">ONE CARIBE</b><div style="color:#555;font-size:13px">Reporte de Inventario — Sucursal: <b>${suc}</b></div></div>
...
async function crearTraslado() {
  ...
  // Verificar que el producto+referencia existe en el catálogo del destino
  showSpinner();
  try {
...
async function confirmarTraslado(id, sucDest, prod, ref, pac, lote) {
  ...
  showSpinner();
  try {
    // Actualizar inventario del destino en la colección correcta
    ...
    // Registrar como entrada en la sucursal destino para que aparezca en Historial
    await db.collection("entradas").add({
      fecha: new Date().toLocaleString(), fechaISO: new Date().toISOString(),
